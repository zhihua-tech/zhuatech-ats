...
const fields = [{"key":"candidateNo","label":"候选人编号","type":"text"},{"key":"email","label":"邮箱","type":"email"},{"key":"privacyConsent","label":"隐私授权","type":"boolean"},{"key":"yearsExperience","label":"工作年限","type":"number"},{"key":"minimumYears","label":"最低年限","type":"number"},{"key":"skills","label":"候选人技能（逗号分隔）","type":"list"},{"key":"mustHaveSkills","label":"必备技能（逗号分隔）","type":"list"},{"key":"resumeScore","label":"简历评分","type":"number"},{"key":"assessmentScore","label":"测评评分","type":"number"},{"key":"interviewScore","label":"面试评分","type":"number"}]
const form = reactive({"candidateNo":"C-001","email":"candidate@example.com","privacyConsent":true,"yearsExperience":6,"minimumYears":3,"skills":"Java,MySQL,Vue","mustHaveSkills":"Java,MySQL","resumeScore":85,"assessmentScore":90,"interviewScore":80})
...
async function run() {
  busy.value = true
  error.value = ''
  try { result.value = await request('/api/enterprise/ats/screen-candidate', { method: 'POST', body: JSON.stringify(payload()) }) }
...
<template>
  <section class="rule-head">
    <div><span>DOMAIN CONTROL</span><h3>候选人结构化筛选</h3><p>按隐私授权、工作年限、必备技能和评分卡形成可解释的筛选结论。</p></div>
    <b>服务端强校验</b>
  </section>
